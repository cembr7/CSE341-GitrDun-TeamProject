 ...
 * In test mode we also include the stack trace to make debugging easier.
 */
/*function sendError(res, err, status = 400) {
  console.error('🛑 UserController error:', err);   // appears in test output
  const payload = {
    error: true,
    message: err.message || 'Bad Request',
  };
  if (process.env.NODE_ENV === 'test') {
    payload.stack = err.stack;   // helpful when running Jest
  }
...
/*exports.createUser = async (req, res) => {
  try {
    ...
    // Basic sanity check – you can rely on the validator in prod,
    // but keep it here for the test‑mode fallback.
    ...
    // Duplicate‑key (unique email) errors have code 11000
    if (err.code === 11000) {
...
/*exports.getUserById = async (req, res) => {
  try {
    ...
    // Invalid ObjectId format throws a CastError → treat as Bad Request
...
/*exports.updateUser = async (req, res) => {
  try {
    ...
    // If the password is being changed, hash it first
    if (updates.password) {
